...
- The SDK now compiles for apps with an iOS 14 or later deployment target, and with Xcode 27, where iOS 15 is the lowest deployment target the SDK allows. The iOS 13 fallback views and property wrappers were annotated `obsoleted: 14`, which made the `#available` fallbacks that reference them fail to build with "is unavailable in iOS".
- Fixed two further Xcode 27 build failures: the now main-actor-isolated `AVSampleBufferDisplayLayer` conformance, and a multi-child `ViewBuilder` in an `#available` fallback that the iOS 27 SDK resolves to a `TupleContent` whose `View` conformance requires iOS 26.
...
### ✅ Added
- Swift 6 support [#1031](https://github.com/GetStream/stream-video-swift/pull/1031)
...
### 🔄 Changed
- The initializer of `AudioSessionConfiguration` is now marked as `public` to allow custom `AudioSessionPolicy` implementations. [`#1025`](https://github.com/GetStream/stream-video-swift/pull/1025)
