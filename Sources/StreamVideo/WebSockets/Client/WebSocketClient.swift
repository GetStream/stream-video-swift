...
class WebSocketClient {
    ...
    // TODO: this decoder is no longer needed because we use the code generated by openapi specs
...
extension WebSocketClient: WebSocketEngineDelegate {
    ...
    func webSocketDidReceiveMessage(_ data: Data) {
        ...
        do {
            event = try StreamJSONDecoder.default.decode(VideoEvent.self, from: data)
        } catch {
            log.error(error, subsystems: .webSocket)
            return
        }

        switch event {
        case let .typeHealthCheckEvent(healthCheckEvent):
            handle(healthCheckEvent: healthCheckEvent)
        case let .typeConnectionErrorEvent(connectionErrorEvent):
            log.info("received an error from WS with code: \(connectionErrorEvent.error?.code ?? -1)")
            connectionErrorEvent.error?.isTokenExpiredError
            // TODO: pass the error properly now so we can handle this correctly (eg. permanent failure or token refresh)
