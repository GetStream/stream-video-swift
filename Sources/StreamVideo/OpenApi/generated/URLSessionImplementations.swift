...
internal typealias OpenAPIClientAPIChallengeHandler =
    ((URLSession, URLSessionTask, URLAuthenticationChallenge) -> (URLSession.AuthChallengeDisposition, URLCredential?))
...
internal class URLSessionRequestBuilder<T>: RequestBuilder<T> {
    ...
    fileprivate func processRequestResponse(
        ...
        guard httpResponse.isStatusCodeSuccessful else {
            completion(.failure(
                ErrorResponse
                    .error(httpResponse.statusCode, data, response, DecodableRequestBuilderError.unsuccessfulHTTPStatusCode)
            ))
            return
        }
        ...
        default:
            ...
            guard let data = data, !data.isEmpty else {
                if let E = T.self as? ExpressibleByNilLiteral.Type {
                    completion(.success(Response(response: httpResponse, body: E(nilLiteral: ()) as! T)))
                } else {
                    completion(.failure(
                        ErrorResponse
                            .error(httpResponse.statusCode, nil, response, DecodableRequestBuilderError.emptyDataResponse)
                    ))
                }
                return
            }
