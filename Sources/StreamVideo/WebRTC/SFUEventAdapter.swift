...
final class SFUEventAdapter {
    ...
    private func handle(
        ...
        guard
            event.participant.userID != Self.recordingUserId
        else {
            return
        }
        ...
        let showTrack = participants.count < Self.participantsThreshold
    ...
    private func handle(
        ...
        let participants = event
            .callState
            .participants
            .filter { $0.userID != Self.recordingUserId }
        ...
        //        stateAdapter.participantsUpdateSubject.send(callParticipants)
    ...
    private func handle(
        ...
        case .video:
            ...
            await stateAdapter.didUpdateParticipants(participants)
        case .screenShare:
            participants[sessionID] = participant.withUpdated(screensharing: true)
            await stateAdapter.didUpdateParticipants(participants)
        default:
            break
        }
