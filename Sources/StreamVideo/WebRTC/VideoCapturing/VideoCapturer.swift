        ...
        try await withCheckedThrowingContinuation { [weak self, videoCapturer] continuation in
            ...
            if outputFormatDimensions.area != preferredDimensions.area {
                videoSource.adaptOutputFormat(
                    ...
                    fps: Int32(outputFormat.clampedToFrameRateRange(configuration.frameRate))
                )
                log.debug(
            ...
            videoCapturer.startCapture(
                with: device,
                format: outputFormat,
                fps: outputFormat.clampedToFrameRateRange(configuration.frameRate)
            ) { [weak self] error in
                if let error {
            ...
            log.debug(
                ...
                OutputFormat:
                    ...
                    FrameRate: \(outputFormat.clampedToFrameRateRange(configuration.frameRate))
                """,
                subsystems: .webRTC
            )
        }
...
extension AVCaptureDevice.Format {
    ...
    func clampedToFrameRateRange(_ value: Int) -> Int {
        let frameRateRange = fpsRange()

        guard !frameRateRange.contains(value) else {
            return value
        }

        return value.clamped(to: frameRateRange)
    }
}
