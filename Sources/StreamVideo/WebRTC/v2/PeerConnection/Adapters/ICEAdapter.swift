...
actor ICEAdapter: @unchecked Sendable {
    ...
    func trickle(_ candidate: RTCIceCandidate) {
        Task {
            do {
                ...
                log.debug(
                    """
                    PeerConnection type:\(peerType) generated candidate while remoteDescription is \(peerConnection.remoteDescription == nil ? "nil" : "non-nil").
                    Candidate: \(candidate)
                    """,
    ...
    private func configure() async {
        ...
        peerConnection
            ...
            .sinkTask { [weak self] in await self?.trickle($0.candidate) }
            .store(in: disposableBag)

        peerConnection
            .publisher(eventType: RTCPeerConnection.HasRemoteDescription.self)
            .log(.debug, subsystems: .iceAdapter)
            .sinkTask { [weak self] _ in await self?.addTrickledICECandidates() }
            .store(in: disposableBag)
        ...
        sfuAdapter
            ...
            .sinkTask { [weak self] in await self?.handleICETrickle($0) }
            .store(in: disposableBag)

        sfuAdapter
            .refreshPublisher
            .sinkTask { [weak self] in await self?.configure() }
            .store(in: disposableBag)
    }
}
