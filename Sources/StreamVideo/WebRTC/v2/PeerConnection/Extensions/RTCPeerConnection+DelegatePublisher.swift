    ...
    var subject: PassthroughSubject<RTCPeerConnectionEvent, Never> {
        withUnsafePointer(to: &AssociatedKeys.delegatePublisher) { key in
            if let existingPublisher = objc_getAssociatedObject(
                ...
                return existingPublisher.publisher
            } else {
                ...
                return newPublisher.publisher
            }
        }
    }

    var dispatchQueue: DispatchQueue {
        withUnsafePointer(to: &AssociatedKeys.dispatchQueue) { key in
            if let existing = objc_getAssociatedObject(
                ...
                let newValue = DispatchQueue(label: "io.getstream.peerconnection")
                objc_setAssociatedObject(
                    self,
                    key,
                    newValue,
    ...
    var publisher: AnyPublisher<RTCPeerConnectionEvent, Never> {
        withUnsafePointer(to: &AssociatedKeys.delegatePublisher) { key in
            if let existingPublisher = objc_getAssociatedObject(
                ...
                self.delegate = newPublisher
                return newPublisher.publisher.receive(on: dispatchQueue).eraseToAnyPublisher()
            }
        }
    }
