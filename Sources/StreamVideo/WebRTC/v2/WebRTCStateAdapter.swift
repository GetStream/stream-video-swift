...
actor WebRTCStateAdapter: ObservableObject {
    ...
    func set(publisher value: RTCPeerConnectionCoordinator) { self.publisher = value }
    func set(subscriber value: RTCPeerConnectionCoordinator) { self.subscriber = value }
    ...
    func didUpdate(sfuAdapter: SFUAdapter?) async throws {
        ...
        publisher
            ...
            .sinkTask { [weak self] in await self?.peerConnectionReceivedTrackEvent(.publisher, event: $0) }
    ...
    private func assignTracksToParticipants(
        ...
        Muted tracks:
        AudioTracks: \(audioTracks.filter { !$0.value.isEnabled }.map(\.key))
        VideoTracks: \(videoTracks.filter { !$0.value.isEnabled }.map(\.key))
        ScreenShareTracks: \(screenShareTracks.filter { !$0.value.isEnabled }.map(\.key))
...
extension AudioSettings {
    init() {
        self.accessRequestEnabled = false
        self.defaultDevice = .unknown
        self.micDefaultOn = false
        self.noiseCancellation = nil
        self.opusDtxEnabled = false
        self.redundantCodingEnabled = false
        self.speakerDefaultOn = false
    }
}
