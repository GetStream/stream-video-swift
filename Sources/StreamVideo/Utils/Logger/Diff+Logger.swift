...
typealias IndentationType = Diff.IndentationType

private struct DifferenceNameLabels {
    let expected: String
    let received: String
    let missing: String
    let extra: String

    init(expected: String, received: String, missing: String, extra: String) {
               ...
               let receivedDict = received as? [AnyHashable: Any] {
                ...
                if (!missingKeys.isEmpty) {
                    ...
                    missingKeys.forEach { key in
                        missingKeyPairs
                            .append(Line(
                                contents: "\(key.description): \(String(describing: expectedDict[key]))",
                    ...
                    resultLines
                        .append(Line(
                            contents: "\(nameLabels.missing) key pairs:",
                            indentationLevel: level,
                            canBeOrdered: false,
                            children: missingKeyPairs
                        ))
                }
                if (!extraKeys.isEmpty) {
                    ...
                    extraKeys.forEach { key in
                        extraKeyPairs
                            .append(Line(
                                contents: "\(key.description): \(String(describing: receivedDict[key]))",
                                indentationLevel: level + 1,
                                canBeOrdered: true
                            ))
                    }
                    resultLines
                        .append(Line(
                            contents: "\(nameLabels.extra) key pairs:",
                            indentationLevel: level,
                            canBeOrdered: false,
                            children: extraKeyPairs
                        ))
                }
                return resultLines
            }
...
private struct Line {
    init(contents: String, indentationLevel: Int, children: [Line], canBeOrdered: Bool) {
...
private func diff<T>(
    ...
    nameLabels: DifferenceNameLabels = .comparing
) -> [String] {
