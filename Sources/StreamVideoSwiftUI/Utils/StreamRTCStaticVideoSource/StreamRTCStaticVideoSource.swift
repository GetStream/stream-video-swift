...
final class StreamRTCStaticVideoSource: @unchecked Sendable {
    ...
    var contentSize: CGSize = .init(width: 640, height: 480) {
        didSet { cachedContent = nil }
    }

    var contentProvider: (CallParticipant, CGSize) async -> CVPixelBuffer? = { participant, size in
        await Task { @MainActor in
            ...
            .toPixelBuffer(contentSize: size)
        }.value
    }
    ...
    private var cachedContent: CVPixelBuffer?
    ...
    private func publishFrame() async {
        ...
        if let cachedContent {
            renderingSubject.send(frame(from: cachedContent))
        } else if let content = await contentProvider(participant, contentSize) {
            renderingSubject.send(frame(from: content))
