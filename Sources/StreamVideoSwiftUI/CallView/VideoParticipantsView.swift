...
public struct VideoCallParticipantView: View {
    ...
    public var body: some View {
        ...
        .onReceive(call?.state.$pipSource) { output in
            if participant.id == output?.id {
                self.feedFrames = { buffer in
                    self.call?.state.feedBuffer(buffer)
                }
            } else {
                self.feedFrames = nil
            }
    ...
    @State private var feedFrames: ((CMSampleBuffer) -> ())?
