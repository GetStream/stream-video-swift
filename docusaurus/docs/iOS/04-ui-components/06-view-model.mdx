...
You can start a call using the method `startCall(callId: String, type: String, members: [MemberRequest], ring: Bool)`, where the parameters are:
...
If you want to display a lobby screen before the user joins the call, you should use the `enterLobby(callId: String, type: String, members: [MemberRequest])` method. This will change the calling state to `.lobby`. When that happens, you can either display your custom implementation of a lobby view, or use the one from the SDK.
