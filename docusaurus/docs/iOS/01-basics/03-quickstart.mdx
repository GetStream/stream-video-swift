...
:::note
In a production application we would call the `.connect()` function earlier during our login flow. For the sake of this tutorial we're simplifying this here. The way we retrieve the `streamVideo` object here is through the `@Injected` property wrapper (read more on [this page](../../guides/dependency-injection).)
:::
