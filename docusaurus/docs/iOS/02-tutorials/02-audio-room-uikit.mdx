...
### Step 1 - Create a new UIKit Application in Xcode
...
3. Select "StreamVideo" and "StreamVideoUIKit" and then click Add Package
...
2. Remove the `Storyboard Name` etnry from your `Info.plist` inside `Application Scene Manifest > Scene Configuration > Window Application Session Role > Item 0`
...
final class MicButtonView: UIButton {
    ...
    init(microphone: MicrophoneManager) {
        super.init(frame: .zero)
        setImage(.init(systemName: microphone.status == .enabled ? "mic.circle" : "mic.slash.circle"), for: .normal)
    }
}
```
