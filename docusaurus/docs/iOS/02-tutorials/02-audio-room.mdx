...
![Preview of the final result](../assets/audioroom_tutorial/notfound.png)
        ...
        Group {
            if callCreated {
                DescriptionView(
                    ...
                    participants: call.state.participants.values.map {$0}
                )
                ParticipantsView(
                    participants: call.state.participants.values.map {$0}
                )
...
#### Room Backstage & Live mode control
...
```swift
call.state.permissionRequests.value.forEach { request in
    // reject the request
    request.reject()
    // grant the request
