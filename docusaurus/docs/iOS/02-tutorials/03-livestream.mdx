...
var body: some View {
    VStack {
        HStack {
            Text("Live")
                .opacity(call.state.callData?.broadcasting == true ? 1 : 0)
        }
        ...
        ZStack {
            HStack {
                if call.state.callData?.broadcasting == true {
                    Button {
                        Task {
                            loading = true
                            try await call.stopBroadcasting()
    ...
    .onChange(of: call.state.callData?.broadcasting, perform: { state in
        loading = false
    })
