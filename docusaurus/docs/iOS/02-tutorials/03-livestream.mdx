...
var body: some View {
    VStack {
        HStack {
            Text("Live")
                .opacity(isBroadcasting == true ? 1 : 0)
        }
...
We are listening for changes to the broadcasting state of the call by setting up a subscription task on `call`s event `AsyncStream`. We are interested only for the events that relate to broadcasting (`typeCallBroadcastingStartedEvent` & `typeCallBroadcastingStoppedEvent`).
...
When this method is called, it takes few seconds until the broadcasting is started. You can show a loading spinner until you receive the `.typeCallBroadcastingStartedEvent` event.
