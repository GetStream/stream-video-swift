...
:::warning
As we want to send the snapshot using WebRTC's internal event channel, we are affected by some limitations. The size of each event cannot surpasses the 5KB. For this reason, we are reducing the snapshot's quality before sending it.
:::
