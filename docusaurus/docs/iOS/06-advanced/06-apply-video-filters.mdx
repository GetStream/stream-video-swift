...
The StreamVideo SDK also supports audio processing of the local track. This opens up possibilities for noise cancellation, voice changing or other audio effects.

If you want to have audio processing, you need to either provide your own implementation of the `AudioProcessingModule` protocol in our `VideoConfig` or use the provided one implemented on `StreamAudioFilterProcessingModule`. 
...
In this code, we are using several newly defined objects, so let's go through them. 

Firstly, we create a `StreamAudioFilterProcessingModule`. This in object provided by StreamVideo SDK, which implements the  `RTCAudioCustomProcessingDelegate`. Internally, it knows how to manage audioFilter lifecycle and forward audio processing requests.

Secondly, we are referring to an `AudioFilter` protocol which is defined in the `StreamVideo SDK` and represents an object that can be used to process audio buffers.
