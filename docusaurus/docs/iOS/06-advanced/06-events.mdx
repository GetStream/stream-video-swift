...
In most cases you can simply use the Async Stream objects the SDK exposes. However for some customizations you'll want to listen to the underlying events that power these stream objects.
...
```swift
Task {
    val call = streamVideo.call("default", "123")
...
|`WSCallEvent`|This is just a placeholder for all client events|
