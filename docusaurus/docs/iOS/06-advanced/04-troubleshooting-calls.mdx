...
Connection issues usually happen when you provide an invalid token when setting up the SDK. When this happens, a web socket connection can't be established with our backend, resulting in errors when trying to connect to a call.
...
- check if have created push providers and you specified their correct names when creating the SDK
