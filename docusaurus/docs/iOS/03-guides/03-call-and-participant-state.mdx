...
The `CallParticipant` is the most essential component used to render a participant in a call. It contains all of the information to render a participant, such as audio & video renderers, availabilities of audio & video, the screen sharing session, reactions, and etc. Here's how you can subscribe to participants updates:
...
The participants that are publishing video, audio or screensharing are always prioritized in the list over regular viewers.

The total number of participants is updated realtime via the health check events. This value is available from the call state's `participantCount` property.
