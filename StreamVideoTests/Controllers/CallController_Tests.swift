...
final class CallController_Tests: StreamVideoTestCase, @unchecked Sendable {
    ...
    func test_setCall_speakingWhileMutedAlreadyTrue_updatesCallState() async throws {
        let call = await MockCall(.dummy())
        _ = subject
    ...
    func test_setCall_ownCapabilitiesUpdatedOnStateAdapter_emptyAfterNonEmpty_updatesCallState() async throws {
        let call = await MockCall(.dummy())
        _ = subject
        subject.call = call
        ...
        await fulfilmentInMainActor { call.state.ownCapabilities.isEmpty }
    }
    ...
    func test_joinCall_coordinatorTransitionsToConnecting() async throws {
        ...
        let joinPolicy = WebRTCJoinPolicy.peerConnectionReadinessAware
