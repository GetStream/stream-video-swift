...
final class CallCRUDTests: IntegrationTest, @unchecked Sendable {
    ...
    private func executeLeaveCallLifecycleScenario(
        ...
        for iteration in 0..<cycles {
            let callId = "leave-\(routeVariant.rawValue)\(iteration)\(UUID().uuidString.prefix(8))"
            ...
            _ = try await call.join(
                callSettings: CallSettings(
                    audioOn: true,
                    speakerOn: routeVariant == .speakerEnabled
                )
            )
    ...
    func test_queryChannels() async throws {
        ...
        await fulfilmentInMainActor { [fetchedCall] in
            fetchedCall.state.custom[colorKey] == blue
        }
