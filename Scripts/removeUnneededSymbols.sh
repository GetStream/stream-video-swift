#!/usr/bin/env bash
#
# Usage: ./removeUnneededSymbols.sh StreamVideo ./Products
#
# Creating an xcframework for StreamVideo generates a .bcsymbolmap file for itself, and one for
# each of its dependencies too (eg. StreamChat). That means that we will end up having something like:
#
# -> StreamVideo/BCSymbolMaps/
#                              <UUID-StreamVideo>.bcsymbolmap
#                              <UUID-StreamVideoSwiftUI>.bcsymbolmap
#
# When adding both StreamChat and StreamVideo to an app, it will throw an error when trying to compile
